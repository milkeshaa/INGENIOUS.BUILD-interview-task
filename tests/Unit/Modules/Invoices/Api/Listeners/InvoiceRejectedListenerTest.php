...
/** @coversDefaultClass \App\Modules\Invoices\Api\Listeners\InvoiceRejectedListener */
class InvoiceRejectedListenerTest extends TestCase
{
    use DatabaseTransactions;

    private InvoiceRejectedListener $listener;
    ...
    public function setUp(): void
    {
        ...
        $this->listener = app()->make(InvoiceRejectedListener::class);
    ...
    public function test_handle_invoice_reject()
    {
        ...
        $this->listener->handle(event: new EntityRejected(approvalDto: $approvalDto));

        $invoiceFromDb = DB::table('invoices')->where('id', $this->invoice->getId())->first();
        self::assertEquals(StatusEnum::REJECTED->value, $invoiceFromDb->status);
    }
}
