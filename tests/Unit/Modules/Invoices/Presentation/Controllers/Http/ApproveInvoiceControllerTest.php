...
use App\Modules\Invoices\Presentation\Controllers\Http\ApproveInvoice;
use Faker\Factory;
...
/** @coversDefaultClass \App\Modules\Invoices\Presentation\Controllers\Http\ApproveInvoice */
class ApproveInvoiceControllerTest extends TestCase
{
    ...
    {
        Event::fake();
        $controller = app()->make(ApproveInvoice::class);

        // real invoice in draft status
        $request = Request::create("/api/invoices/{$this->invoice->getId()->toString()}/approve");
        $request->setRouteResolver(function () use ($request) {
            return (new Route('PATCH', '/api/invoices/{invoice_id}/approve', []))->bind($request);
        });
        ...
        // fake uuid
        $uuid = Uuid::uuid4();
        $request = Request::create("/api/invoices/{$uuid}/approve");
        $request->setRouteResolver(function () use ($request) {
            return (new Route('PATCH', '/api/invoices/{invoice_id}/approve', []))->bind($request);
        });
