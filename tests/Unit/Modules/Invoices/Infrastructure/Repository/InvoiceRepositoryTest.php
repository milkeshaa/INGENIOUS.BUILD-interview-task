<?php

declare(strict_types=1);

namespace Tests\Unit\Modules\Invoices\Infrastructure\Repository;

use App\Domain\Company\Entity as Company;
use App\Domain\Enums\StatusEnum;
use App\Domain\Invoice\Entity as Invoice;
use App\Domain\Invoice\InvoiceAggregate;
use App\Domain\Product\Entity as Product;
use App\Domain\Product\ProductLineEntity;
use App\Domain\Shared\ValueObject\Currency\Currency;
use App\Domain\Shared\ValueObject\Email\Email;
use App\Domain\Shared\ValueObject\Email\Exceptions\InvalidEmailException;
use App\Domain\Shared\ValueObject\Phone\Exceptions\InvalidPhoneException;
use App\Domain\Shared\ValueObject\Phone\Phone;
use App\Domain\Shared\ValueObject\Price\Exceptions\InvalidPriceException;
use App\Domain\Shared\ValueObject\Price\Price;
use App\Domain\Shared\ValueObject\Quantity\Exceptions\InvalidQuantityException;
use App\Domain\Shared\ValueObject\Quantity\Quantity;
use App\Modules\Invoices\Infrastructure\Repository\Dto\InvoiceUpdateDto;
use App\Modules\Invoices\Infrastructure\Repository\InvoiceRepository;
use Faker\Factory;
use Illuminate\Foundation\Testing\DatabaseTransactions;
use Illuminate\Support\Carbon;
use Illuminate\Support\Collection;
use Illuminate\Support\Facades\DB;
use Tests\TestCase;
use Ramsey\Uuid\Uuid;

/** @coversDefaultClass \App\Modules\Invoices\Infrastructure\Repository\InvoiceRepository */
class InvoiceRepositoryTest extends TestCase
{
    use DatabaseTransactions;

    private InvoiceRepository $repository;
    private InvoiceAggregate $invoiceAggregate;
    private Invoice $invoice;

    /**
     * @throws InvalidPriceException
     * @throws InvalidEmailException
     * @throws InvalidPhoneException
     * @throws InvalidQuantityException
     */
    public function setUp(): void
    {
        parent::setUp();

        $this->repository = new InvoiceRepository();

        $faker = Factory::create();
        $productRazer = new Product(
            id: Uuid::uuid4(),
            name: 'razer',
            price: new Price(price: 1000),
            currency: new Currency(currency: 'usd'),
            createdAt: null,
            updatedAt: null,
        );
        $productPepsi = new Product(
            id: Uuid::uuid4(),
            name: 'pepsi',
            price: new Price(price: 1200),
            currency: new Currency(currency: 'usd'),
            createdAt: null,
            updatedAt: null,
        );
        $pl1 = new ProductLineEntity(
            id: Uuid::uuid4(),
            invoice: null,
            product: $productRazer,
            quantity: new Quantity(quantity: 12),
            createdAt: null,
            updatedAt: null,
        );
        $pl2 = new ProductLineEntity(
            id: Uuid::uuid4(),
            invoice: null,
            product: $productPepsi,
            quantity: new Quantity(quantity: 5),
            createdAt: null,
            updatedAt: null,
        );
        $this->invoiceAggregate = new InvoiceAggregate(
            id: Uuid::uuid4(),
            number: Uuid::uuid4(),
            date: Carbon::now(),
            dueDate: Carbon::now(),
            company: new Company(
                id: Uuid::uuid4(),
                name: $faker->company(),
                street: $faker->streetAddress(),
                city: $faker->city(),
                zip: $faker->postcode(),
                phone: new Phone(phone: $faker->phoneNumber()),
                email: new Email(email: $faker->email()),
                createdAt: null,
                updatedAt: null,
            ),
            productLines: Collection::make([
                $pl1,
                $pl2,
            ]),
            status: StatusEnum::APPROVED,
            createdAt: Carbon::now(),
            updatedAt: Carbon::now()
        );
        $this->invoice = new Invoice(
            id: $this->invoiceAggregate->getId(),
            number: $this->invoiceAggregate->getNumber(),
            date: $this->invoiceAggregate->getDate(),
            dueDate: $this->invoiceAggregate->getDueDate(),
            companyId: $this->invoiceAggregate->getCompany()->getId(),
            status: $this->invoiceAggregate->getStatus(),
            createdAt: $this->invoiceAggregate->getCreatedAt(),
            updatedAt: $this->invoiceAggregate->getUpdatedAt(),
        );
        DB::table('companies')->insert([
            'id' => $this->invoiceAggregate->getCompany()->getId(),
            'name' => $this->invoiceAggregate->getCompany()->getName(),
            'street' => $this->invoiceAggregate->getCompany()->getStreet(),
            'city' => $this->invoiceAggregate->getCompany()->getCity(),
            'zip' => $this->invoiceAggregate->getCompany()->getZip(),
            'phone' => $this->invoiceAggregate->getCompany()->getPhone()->toString(),
            'email' => $this->invoiceAggregate->getCompany()->getEmail()->toString(),
            'created_at' => $this->invoiceAggregate->getCompany()->getCreatedAt(),
            'updated_at' => $this->invoiceAggregate->getCompany()->getUpdatedAt(),
        ]);
        DB::table('products')->insert([
            'id' => $productPepsi->getId(),
            'name' => $productPepsi->getName(),
            'price' => $productPepsi->getPrice()->getPrice(),
            'currency' => $productPepsi->getCurrency()->toString(),
            'created_at' => $productPepsi->getCreatedAt(),
            'updated_at' => $productPepsi->getUpdatedAt(),
        ]);
        DB::table('products')->insert([
            'id' => $productRazer->getId(),
            'name' => $productRazer->getName(),
            'price' => $productRazer->getPrice()->getPrice(),
            'currency' => $productRazer->getCurrency()->toString(),
            'created_at' => $productRazer->getCreatedAt(),
            'updated_at' => $productRazer->getUpdatedAt(),
        ]);
        DB::table('invoices')->insert([
            'id' => $this->invoice->getId(),
            'number' => $this->invoice->getNumber(),
            'date' => $this->invoice->getDate(),
            'due_date' => $this->invoice->getDueDate(),
            'company_id' => $this->invoice->getCompanyId(),
            'status' => $this->invoice->getStatus(),
            'created_at' => $this->invoice->getCreatedAt(),
            'updated_at' => $this->invoice->getUpdatedAt(),
        ]);
        DB::table('invoice_product_lines')->insert([
            'id' => $pl1->getId(),
            'invoice_id' => $this->invoice->getId(),
            'product_id' => $pl1->getProduct()->getId(),
            'quantity' => $pl1->getQuantity()->getNumber(),
            'created_at' => $pl1->getCreatedAt(),
            'updated_at' => $pl1->getUpdatedAt(),
        ]);
        DB::table('invoice_product_lines')->insert([
            'id' => $pl2->getId(),
            'invoice_id' => $this->invoice->getId(),
            'product_id' => $pl2->getProduct()->getId(),
            'quantity' => $pl2->getQuantity()->getNumber(),
            'created_at' => $pl2->getCreatedAt(),
            'updated_at' => $pl2->getUpdatedAt(),
        ]);
    }

    public function test_update_method()
    {
        $dto = new InvoiceUpdateDto(
            number: Uuid::uuid4()->toString(),
            date: Carbon::now()->addDay()->toDateTimeString(),
            dueDate: Carbon::now()->addYear()->toDateTimeString(),
            companyId: $this->invoice->getCompanyId()->toString(),
            status: StatusEnum::DRAFT->value,
            createdAt: Carbon::now()->subYear()->toDateTimeString(),
            updatedAt: Carbon::now()->subYear()->toDateTimeString()
        );
        $result = $this->repository->update(invoiceId: $this->invoice->getId(), invoice: $dto);
        self::assertEquals(1, $result);
        $invoice = DB::table('invoices')->where('id', $this->invoice->getId())->first();
        self::assertNotNull($invoice);
        self::assertNotEquals($invoice->number, $this->invoice->getNumber()->toString());
        self::assertNotEquals($invoice->date, $this->invoice->getDate()->toDateTimeString());
        self::assertNotEquals($invoice->due_date, $this->invoice->getDueDate()->toDateTimeString());
        self::assertNotEquals($invoice->status, $this->invoice->getStatus()->value);
        self::assertNotEquals($invoice->created_at, $this->invoice->getCreatedAt()->toDateTimeString());
        self::assertNotEquals($invoice->updated_at, $this->invoice->getUpdatedAt()->toDateTimeString());

        $result = $this->repository->update(invoiceId: Uuid::uuid4(), invoice: $dto);
        self::assertEquals(0, $result);
    }

    public function test_find_method()
    {
        $invoice = $this->repository->find(invoiceId: $this->invoice->getId());
        self::assertNotNull($invoice);
        self::assertEquals($invoice->getNumber(), $this->invoice->getNumber());
        self::assertEquals($invoice->getId(), $this->invoice->getId());
        self::assertEquals($invoice->getDate()->toDateString(), $this->invoice->getDate()->toDateString());
        self::assertEquals($invoice->getDueDate()->toDateString(), $this->invoice->getDueDate()->toDateString());
        self::assertEquals($invoice->getStatus(), $this->invoice->getStatus());
        self::assertEquals($invoice->getCompanyId(), $this->invoice->getCompanyId());
        self::assertEquals($invoice->getCreatedAt()->toDateString(), $this->invoice->getCreatedAt()->toDateString());
        self::assertEquals($invoice->getUpdatedAt()->toDateString(), $this->invoice->getUpdatedAt()->toDateString());

        $invoice = $this->repository->find(invoiceId: Uuid::uuid4());
        self::assertNull($invoice);
    }

    /**
     * @throws InvalidEmailException
     * @throws InvalidPhoneException
     */
    public function test_get_aggregate_method()
    {
        $invoice = $this->repository->getAggregate(invoiceId: Uuid::uuid4());
        self::assertNull($invoice);

        $invoice = $this->repository->getAggregate(invoiceId: $this->invoice->getId());
        self::assertNotNull($invoice);
        self::assertEquals($invoice->getNumber(), $this->invoiceAggregate->getNumber());
        self::assertEquals($invoice->getId(), $this->invoiceAggregate->getId());
        self::assertEquals($invoice->getDate()->toDateString(), $this->invoiceAggregate->getDate()->toDateString());
        self::assertEquals($invoice->getDueDate()->toDateString(), $this->invoiceAggregate->getDueDate()->toDateString());
        self::assertEquals($invoice->getStatus(), $this->invoiceAggregate->getStatus());
        self::assertEquals($invoice->getCompany(), $this->invoiceAggregate->getCompany());
        self::assertEquals($invoice->getProductLines(), $this->invoiceAggregate->getProductLines());
        self::assertEquals($invoice->getCreatedAt()->toDateString(), $this->invoiceAggregate->getCreatedAt()->toDateString());
        self::assertEquals($invoice->getUpdatedAt()->toDateString(), $this->invoiceAggregate->getUpdatedAt()->toDateString());
    }
}
